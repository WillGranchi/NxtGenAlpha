/**
 * Component for listing and selecting Indicator strategies.
 */
...
interface IndicatorStrategyListProps {
  selectedIds: number[];
  onSelectionChange: (ids: number[]) => void;
  onLoadStrategy?: (id: number) => void;
}

export const IndicatorStrategyList: React.FC<IndicatorStrategyListProps> = ({
  selectedIds,
  onSelectionChange,
  onLoadStrategy
}) => {
  const [strategies, setStrategies] = useState<IndicatorStrategy[]>([]);
  ...
  const loadStrategies = async () => {
    try {
      ...
      setStrategies(data.indicator_strategies || []);
    } catch (err: any) {
      setError(err.message || 'Failed to load indicator strategies');
      console.error('Error loading indicator strategies:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (strategies.length === 0) {
    return (
      <div className="p-8 text-center text-text-muted border border-border-default rounded-lg">
        <p>No indicator strategies saved yet.</p>
        <p className="text-sm mt-2">Create strategies in the Dashboard tab to get started.</p>
      </div>
    );
  }
